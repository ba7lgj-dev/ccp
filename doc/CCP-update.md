...
### Update-Index-HistoryOrders
- 首页新增“我的历史订单”模块，调用 /mp/trip/myHistory 展示最近记录（含发起者标记、状态与时间）。
- tabBar 文案调整：大厅入口显示为“拼单大厅”。
- 后端新增 GET /mp/trip/myHistory 接口，按参与记录返回最近20条历史订单。
- 首页与拼单大厅接入认证链 checkAuthChain，确保实名与学校认证流程一致。
