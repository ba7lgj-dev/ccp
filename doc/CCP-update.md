...
### Update-2025-UI-Refresh
- 小程序端界面优化：登录页重构为欢迎式卡片布局，主按钮 loading 禁止重复点击，协议提示高亮。
- 首页统一卡片化风格，增加欢迎区、学校卡片、快捷入口与最近行程预览，历史区支持骨架屏与空态提示。
- 新增“我的订单”快捷入口与学校切换按钮，保持原有接口和跳转逻辑不变。

### Update-2025-XX-FrontRefine
- 全局：app.js 丰富 globalData（用户、认证状态、学校、校区、当前行程、全局 loading），checkAuthChain 统一拦截未登录/未实名/未学校认证场景；http.js 新增 get/post 包装，统一 BASE_URL 拼接与 token 失效跳转；app.wxss 补充常用主题色、按钮、卡片和布局样式。
- 登录页：重新设计视觉与文案，登录按钮支持 loading 防重复，登录完成后调用全局认证链决定跳转。
- 学校/校区：学校选择页改为仅展示已通过认证学校并支持本地搜索、空态跳转认证；校区选择页展示当前学校信息、缺学校时提示并引导返回选择。
...
### Update-Chat-Confirm-Profile (近期修复)
- 小程序聊天页新增短轮询，进入拼单聊天后每隔数秒自动拉取新消息，离开页面自动清理轮询，避免资源浪费。
- 拼车订单满员后，详情页支持发起确认与成员确认，依据订单状态动态展示确认按钮并同步后端状态流转。
- “我的订单/历史订单”页面上线，历史订单仅支持查看与进入详情，已结束行程在详情页不再显示拼车操作。
- 实名通过后，个人资料编辑页锁定真实姓名和性别字段，前端提交时不再覆盖实名认证信息。
- 后台实名审核列表新增身份证性别与用户性别的匹配提示，审核通过前遇到不匹配会弹窗提醒谨慎处理。
- “我的”页新增学校认证入口，跳转至学校认证页以便查看或添加更多学校认证记录。
