...
Page({
  ...
  onShow() {
    ...
    if (app && typeof app.checkAuthChain === 'function') {
      app.checkAuthChain({ from: 'realname', allowAuthPages: true })
    }
    ...
    realAuthService.getRealAuthInfo().then((data) => {
      ...
      if (app && app.globalData) {
        ...
        if (app.globalData.auth) {
          app.globalData.auth.realAuthStatus = status
          app.globalData.auth.hasApprovedSchool = (app.globalData.auth.schoolAuthList || []).some((i) => i.status === 2)
          app.globalData.auth.lastAuthFetchTime = Date.now()
        }
      }
    }).catch((err) => {
